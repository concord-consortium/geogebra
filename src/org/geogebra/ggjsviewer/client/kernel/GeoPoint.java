...
GeoPointInterface {   	
	...
	public void setPointStyle(int style) {
		...
		if (style > -1 && style <= EuclidianViewConstants.MAX_POINT_STYLE)
			pointStyle = style;
		else
			pointStyle = -1;
...
/***********************************************************/
	...
	private StringBuilder buildValueString() { 
    	...
    	if (isInfinite()) {
			sbBuildValueString.append(bApp.getPlain("undefined"));
    ...
    public String getStartPointXML() {
    	...
    	if (isAbsoluteStartPoint()) {		
			...
			sb.append(/*AGUtil.encodeXML(*/getLabel()/*AG)*/);
