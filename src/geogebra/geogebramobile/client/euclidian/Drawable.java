...
public abstract class Drawable extends DrawableND {
	...
	final public void ensureLabelDrawsOnScreen() {
		// draw label and 
		drawLabel(/*AGview.g2Dtemp*/);
		
		// make sure labelRectangle fits on screen horizontally		
		if (xLabel < 3) 
			xLabel = 3;
		else
			xLabel = Math.min(xLabel, view.getOffsetWidth() - labelRectangle.width - 3);					
		if (yLabel < view.fontSize) 
			yLabel = view.fontSize;
		else
			yLabel = Math.min(yLabel, view.getOffsetHeight() - 3);	
	 ...
	 * @param bgColor
	 */
	/*AGfinal void drawMultilineLaTeX(Graphics2D g2, Font font, Color fgColor, Color bgColor) {
		int fontSize = g2.getFont().getSize();
	...
	//AGprivate static geogebra.gui.hoteqn.sHotEqn eqn;
	
	/*AGfinal  public static Dimension drawEquation(Application app, Graphics2D g2, int x, int y, String text, Font font, Color fgColor, Color bgColor)
	{
		Dimension dim;
		if (eqn == null) {
	...
	final protected boolean addLabelOffset(boolean ensureLabelOnScreen) {
		if (ensureLabelOnScreen) {
			...
			int xmax = view.getOffsetWidth() - 15;
			int ymax = view.getOffsetHeight() - 5;
	...
	/*AGfinal public static void drawWithValueStrokePure(Shape shape, Graphics2D g2) {
	...
	*/
    ...
    /*AG		case GeoElement.GEO_CLASS_POINT:
