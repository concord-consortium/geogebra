...
import geogebra.geogebramobile.client.euclidian.clipping.ClipLine;
...
public class DrawVector extends Drawable implements Previewable {
	...
	final public void update() {
		...
		//EJif (v == view.getEuclidianController().recordObject)
		    //EJrecordToSpreadsheet(v);
    ...
    public void draw(/*EJGraphics2D g2*/) {
        if (isVisible) {
        	if (traceDrawingNeeded) {
        		traceDrawingNeeded = false;
        		//EJGraphics2D g2d = view.getBackgroundGraphics();
    			//EJif (g2d != null) drawTrace(g2d);    			
        	}
        	
            if (geo.doHighlighting()) {
                /*EJg2.setPaint(v.getSelColor());
            ...
            if (labelVisible) {
				/*EJg2.setFont(view.fontVector);
				g2.setPaint(v.getLabelColor());
				drawLabel(g2);*/
            	view.setFont(view.fontVector);
				view.setPaint(v.getLabelColor());
				drawLabel();
            }            
        }
    }
	...
	final void drawTrace(/*EJGraphics2D g2*/) {
		/*EJg2.setPaint(v.getObjectColor());
		g2.setStroke(objStroke);  
		if (lineVisible) g2.draw(line);  
		if (arrowheadVisible) g2.fill(gp);*/
	...
	final public void drawPreview(/*EJGraphics2D g2*/) {
		if (isVisible) {		
			...
			*/
			...
			view.setStroke(ConstructionDefaults.colPreview);  
			if (arrowheadVisible) view.fill(gp);                                    
			if (lineVisible) {
        		view.beginPath();
