...
import geogebra.geogebramobile.client.kernel.AlgoAngleLines;
import geogebra.geogebramobile.client.kernel.AlgoAnglePoints;
import geogebra.geogebramobile.client.kernel.AlgoAngleVector;
import geogebra.geogebramobile.client.kernel.AlgoAngleVectors;
...
import geogebra.geogebramobile.client.kernel.gawt.GeneralPath;
	...
	final public void draw(/*AGGraphics2D g2*/) {
		if (isVisible) {
			if (angle.alphaValue > 0.0f && (!show90degrees || view.rightAngleStyle != EuclidianView.RIGHT_ANGLE_STYLE_L)) {
				//AGg2.setPaint(angle.getFillColor());
				//AGg2.fill(shape);
				view.setPaint(angle.getFillColor());
				view.fill(shape);
			}

			if (geo.doHighlighting()) {
				//AGg2.setPaint(angle.getSelColor());
				//AGg2.setStroke(selStroke);
				//AGg2.draw(shape);
				view.setPaint(angle.getSelColor());
			...
			if (geo.lineThickness > 0) {
				//AGg2.setPaint(angle.getObjectColor());
				//AGg2.setStroke(objStroke);
				//AGg2.draw(shape);
				view.setPaint(angle.getObjectColor());
...
// Michael Borcherds 2007-11-19
			...
			if (labelVisible) {
				//AGg2.setPaint(angle.getLabelColor());
				//AGg2.setFont(view.fontAngle);
				view.setPaint(angle.getLabelColor());
				view.setFont(view.fontAngle);
				drawLabel(/*AGg2*/);
			}
		}
	}
