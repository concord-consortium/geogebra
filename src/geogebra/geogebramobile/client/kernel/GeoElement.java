...
import geogebra.geogebramobile.client.kernel.gawt.Point2D;
...
//	 Michael Borcherds END 2007-11-19
	...
	public boolean isRedefineable() {
		return !fixed && bApp.letRedefine() && !(isTextValue() || isGeoImage()) &&
...
//	}	
	...
	private void updateSpreadsheetCoordinates() {				
			...
			&& Character.isDigit(label.charAt(label.length()-1)))  // ends with digit
		{
			...
			//AG ISNT SUPPORTED Matcher matcher = GeoElement.spreadsheetPattern.matcher(getLabel());
			
			int column = getSpreadsheetColumn(/*AGmatcher*/);
			int row = getSpreadsheetRow(/*AGmatcher*/);
			if (column >= 0 && row >= 0) {			
	...
	public static Point getSpreadsheetCoordsForLabel(String inputLabel) {
		// we need to also support wrapped GeoElements like
		// $A4 that are implemented as dependent geos (using ExpressionNode)	
		//AG NOT COMPATIBLE Matcher matcher = GeoElement.spreadsheetPattern.matcher(inputLabel);				
		int column = getSpreadsheetColumn(/*AGmatcher*/);
		int row = getSpreadsheetRow(/*AGmatcher*/);
...
//		}
	...
	public static String getSpreadsheetColumnName(String label) {
		/*AGMatcher matcher = spreadsheetPattern.matcher(label);
	...
	public static boolean isSpreadsheetLabel(String str) {
		/*AGMatcher matcher = spreadsheetPattern.matcher(str);
	...
	// Cong Liu	
	/*AGpublic static final Pattern spreadsheetPattern = 
    ...
    */
	 ...
	 */
	public static void setSpreadsheetCell(BaseApplication app, int row, int col, GeoElement cellGeo) {
		...
		//Application.debug(sb.toString());
			...
			//AGapp.getKernel().getAlgebraProcessor().processAlgebraCommand(sb.toString(), false);
		
			/*AGGeoElement cell = app.getKernel().lookupLabel(cellName);
			if (cell != null) {
			...
			// Cong Liu	
	public static int getSpreadsheetColumn(/*AGMatcher matcher*/) {	
		/*AGif (! matcher.matches()) return -1;
	...
	// Cong Liu	
	public static int getSpreadsheetRow(/*AGMatcher matcher*/) {		
		/*AGif (! matcher.matches()) return -1;
	...
	public static void setLabels(String labelPrefix, GeoElement[] geos) {
		...
		switch (visible) {
			...
			default : 
				...
				if (false) {
				/*AGMatcher matcher = GeoElement.spreadsheetPattern.matcher(labelPrefix);								
				if (matcher.matches()) {
	...
	public String getDefaultLabel() {				
		...
		if (isGeoPoint()) {
			...
			do {
				counter++;
				str = bApp.getPlain("Name.polygon") + counter;
			...
			do {
				counter++;
				str = bApp.getPlain("Name.text") + counter;
			} while (!cons.isFreeLabel(str));
			return str;
		} else if (isGeoImage()) {
			int counter = 0;			
			...
			do {
				counter++;
				str = bApp.getPlain("Name.picture") + counter;
			} while (!cons.isFreeLabel(str));
			return str;
		} else if (isGeoLocus()) {
			int counter = 0;			
			...
			do {
				counter++;
				str = bApp.getPlain("Name.locus") + counter;
			} while (!cons.isFreeLabel(str));
			return str;
		
		/*AG} else if (this instanceof GeoTextField) {
			int counter = 0;			
			String str;
			do {
				counter++;
				str = bApp.getPlain("Name.textfield") + counter;
			} while (!cons.isFreeLabel(str));
			return str;*/
		} else if (isGeoButton()) {
			int counter = 0;			
			...
			do {
				counter++;
				str = bApp.getPlain("Name.button") + counter;
			} while (!cons.isFreeLabel(str));
			return str;
		} else if (isGeoList()) {
			...
			do {
				counter++;
				str = list.isMatrix() ? bApp.getPlain("Name.matrix") + counter : bApp.getPlain("Name.list") + counter;
			} while (!cons.isFreeLabel(str));
			return str;
		}
		else {
	...
	protected void doRemove() {
		...
		bApp.removeSelectedGeo(this, false);					
...
//    }
	...
	public String translatedTypeString() {
		return bApp.getPlain(getTypeString());
	}
	...
	final public String getLongDescriptionHTML(
		...
		if (algoParent == null || isTextValue())
			...
			boolean reverseOrder = bApp.isReverseNameDescriptionLanguage();		
			if (!reverseOrder) {
			...
			if (algoParent != null) {
				...
				boolean rightToLeft = bApp.isRightToLeftReadingOrder(); 
				if (rightToLeft)  
					sbLongDescHTML.append("\u200e\u200f: \u200e"); 
				else
					sbLongDescHTML.append(": ");
	...
	final public String getAlgebraDescription() {
		if (strAlgebraDescriptionNeedsUpdate) {
			if (isDefined()) {
				...
				sbAlgebraDesc.append(bApp.getPlain("undefined"));
	...
	*/
	...
	public String getNameDescription() {
		...
		if (bApp.isReverseNameDescriptionLanguage()) {
			//	reverse order: "A point"
	...
	/*AGfinal public String getXMLtypeString() {		
	 ...
	 */
	/*AGpublic String getI2G(int mode) {
		boolean oldValue = kernel.isTranslateCommandName();
		kernel.setTranslateCommandName(false);
...
//	}
	 ...
	 */
	/*AGpublic String getFormulaString(int ExpressionNodeType, boolean substituteNumbers)
	{
	...
	/*AGpublic String getTraceColumn1() {
		if (bApp.isUsingLayout() && bApp.getGuiManager().showSpreadsheetView() && traceColumn1 == -1) {
			traceColumn1 = bApp.getGuiManager().getHighestUsedSpreadsheetColumn() + 1;
		}
	...
	public int getTraceRow() {
		...
		if (!(bApp.isUsingLayout() && bApp.getGuiManager().showSpreadsheetView())) return -1;
		
		return bApp.getGuiManager().getSpreadsheetTraceRow(traceColumn1);
	}
	...
	/*AGprivate void runGgbScript(String arg) {
		...
		try {
			...
			bApp.showError(bApp.getPlain("ErrorInScriptAtLineA",(i+1)+"")+"\n"+e);
			success = false;
		}
		
		if (success) bApp.storeUndoInfo();
	}
	...
	public void runJavaScript(String arg) {
		...
		if (bApp.isApplet()) {
			if (arg == null) {
				Object [] args = { };
				bApp.getApplet().callJavaScript("ggb"+getLabel(), args);				
			} else {
				Object [] args = { arg };
				bApp.getApplet().callJavaScript("ggb"+getLabel(), args);
			}
		} else {
			bApp.getScriptManager().evalScript(javaScript, arg);
		}
		} catch (Exception e) {
			bApp.showError(bApp.getPlain("ErrorInJavaScript")+"\n"+e);
