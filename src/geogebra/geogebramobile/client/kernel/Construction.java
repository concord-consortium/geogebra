...
public class Construction {
	...
	final public void updateConstruction() {
		/*AGif (!kernel.isMacroKernel() && kernel.app.hasGuiManager())
	...
	private GeoElement autoCreateGeoElement(String label) {		
		...
		// check spreadsheet cells
			...
			/*AGMatcher cellNameMatcher = GeoElement.spreadsheetPattern.matcher(label);
	...
	final public GeoElement createSpreadsheetGeoElement(GeoElement neighbourCell, String label) {	
		...
		// no neighbouring cell: create number with value 0
			...
			result = null; //AGnew GeoNumeric(this);
		}				
...
//			 Application.debug("newGeo: " + newGeo + ", visible: " + newGeo.isEuclidianVisible() + ", algo: " + newGeoAlgo);
		...
		if (pos < 0) {
			//restoreCurrentUndoInfo();
			BaseApplication.debug("replace failed: oldXML string not found:\n" + oldXML);
			//AGthrow new MyError(getBaseApplication(), "ReplaceFailed");
		}
...
//		System.out.println("END redefine.");
	...
	private void buildConstruction(StringBuilder consXML) throws Exception {
		...
		try {
			/*AGif (undoManager == null)
				undoManager = new UndoManager(this);
			undoManager.processXML(consXML.toString());*/
			kernel.notifyReset();
			kernel.updateConstruction();
		} catch (Exception e) {
			//AGrestoreCurrentUndoInfo();
			throw e;
		}
	}	
...
//	}
	...
	public String getConstructionI2G(int mode) {
		...
		try {
			...
			if (mode == CONSTRUCTION) {
				...
				for (int i = 0; i < size; ++i) {
					ce = (ConstructionElement) ceList.get(i);
					//AGsb.append(ce.getI2G(ConstructionElement.ELEMENTS));
				}
				sb.append("\t</elements>\n");
				...
				for (int i = 0; i < size; ++i) {
					ce = (ConstructionElement) ceList.get(i);
					//AGsb.append(ce.getI2G(ConstructionElement.CONSTRAINTS));
				}
				sb.append("\t</constraints>\n");
			} else if (mode == DISPLAY) {
				for (int i = 0; i < size; ++i) {
					ce = (ConstructionElement) ceList.get(i);
					//AGsb.append(ce.getI2G(ConstructionElement.DISPLAY));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 */
	/*AGpublic void addRandomNumber(GeoNumeric num) {
	 ...
	 */
	/*AGpublic void removeRandomNumber(GeoNumeric num) {
		if (randomNumbers != null) 
     ...
     */
   /*AG final public void updateAllRandomNumbers() {    	
